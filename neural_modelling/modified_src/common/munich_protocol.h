...
static inline multicast_packet
    ...
    if (_mode != MUNICH_PROTOCOL_PUSH_BOT) {
        log_mini_error("%u%d", 7001, _mode);
        /* "The mode you configured is not the pushbot, and so this message is invalid for mode %d"*/
...
static inline multicast_packet _key_retina(
    ...
    log_mini_error("%u", 7016);  /* "The no of pixels is not supported in this protocol."*/
    rt_error(RTE_API);
    ...
    if (events_in_key) {
        if (!payload_holds_time_stamps) {
            ...
            if (size_of_time_stamp_in_bytes == 0) {
                ...
                log_mini_error("%u%d", 7017,size_of_time_stamp_in_bytes);
        ...
        log_mini_warning("%u", 7018);
        /* "The current SpyNNaker models do not support the reception of packets with payloads, therefore you will need to add a adaptor model between the device and spynnaker models."*/
        ...
        if (payload_holds_time_stamps ||
            ...
            log_mini_error("%u", 7019);
            /* "If you are using payloads to store events, you cannot have time stamps at all."*/
