...
static inline void do_motion(
    ...
    log_mini_debug("%u%s%d%s%d%d%u", 7402, direction,direction_count, opposite, opposite_count, delta,delta_threshold);
    /* "%s = %d, %s = %d, delta = %d, threshold = %u"*/

    if (delta >= delta_threshold) {
        log_mini_debug("%u%s", 7403, direction);  /* "Moving %s"*/
        last_speed[direction_index - 1] = speed;
        last_speed[opposite_index - 1] = 0;
        send(direction_index, speed);
    } else if (delta <= -delta_threshold) {
        log_mini_debug("%u%s", 7404, direction);  /* "Moving %s"*/
        ...
        log_mini_debug("%u%s%s", 7405, direction,opposite);
...
static inline void do_update(
    ...
    if (delta > 0) {
        log_mini_debug("%u%s%d", 7406, direction, direction_speed);  /* "Resending %s = %d"*/
        send(direction_index, direction_speed);
    } else if (delta < 0) {
        log_mini_debug("%u%s%d", 7407, opposite, opposite_speed);  /* "Resending %s = %d"*/
        send(opposite_index, opposite_speed);
    } else {
        log_mini_debug("%u%s%s", 7408, direction,opposite);
        /* "Resending No Motion in the %s-%s direction"*/
        send(direction_index, 0);
    }
}
...
void timer_callback(uint unused0, uint unused1) {
    ...
    log_mini_debug("%u%d", 7409, time);  /* "Timer tick %d"*/

    if ((infinite_run != TRUE) && (time == simulation_ticks)) {
        simulation_handle_pause_resume(NULL);
        log_mini_info("%u", 7410);  /* "Simulation complete.\n"*/
    ...
    while (in_spikes_get_next_spike(&s)) {
        ...
        if (nid < N_COUNTERS) {
            ...
            log_mini_debug("%u%d", 7411, nid);  /* "Received spike from unknown neuron %d"*/
        }
    }
...
void read_parameters(address_t region_address) {
    log_mini_info("%u%.8x", 7412, region_address);  /* "Reading parameters from 0x%.8x"*/
    ...
    log_mini_info("%u%d%d%d%d%d%d%d", 7413,key, speed, sample_time, update_time, delay_time, delta_threshold,continue_if_not_different);
    /* "Key = %d, speed = %d, sample_time = %d, update_time = %d, delay_time = %d, delta_threshold = %d, continue_if_not_different = %d"*/
...
void incoming_spike_callback(uint key, uint payload) {
    ...
    log_mini_debug("%u%x%d", 7414, key, time);  /* "Received spike %x at time %d\n"*/
...
static bool initialize(uint32_t *timer_period) {
    log_mini_info("%u", 7415);  /* "initialise: started"*/
    ...
    log_mini_info("%u", 7416);  /* "initialise: completed successfully"*/
...
void c_main(void) {
    ...
    if (!initialize(&timer_period)) {
        log_mini_error("%u", 7417);  /* "Error in initialisation - exiting!"*/
        rt_error(RTE_SWERR);
    }
