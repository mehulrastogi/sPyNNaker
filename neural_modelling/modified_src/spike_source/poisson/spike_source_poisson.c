...
void print_spike_sources(){
    for (index_t s = 0; s < global_parameters.n_spike_sources; s++) {
        log_mini_info("%u%d", 7418, s);  /* "atom %d"*/
        log_mini_info("%u%u", 7419, poisson_parameters[s].start_ticks);  /* "scaled_start = %u"*/
        log_mini_info("%u%u", 7420, poisson_parameters[s].end_ticks);  /* "scaled end = %u"*/
        log_mini_info("%u%d", 7421, poisson_parameters[s].is_fast_source);  /* "is_fast_source = %d"*/
        log_mini_info("%u%k", 7422,(REAL)(poisson_parameters[s].exp_minus_lambda));
        /* "exp_minus_lamda = %k"*/

        log_mini_info("%u%k", 7423, poisson_parameters[s].mean_isi_ticks);  /* "isi_val = %k"*/
        log_mini_info("%u%k", 7424, poisson_parameters[s].time_to_spike_ticks);
        /* "time_to_spike = %k"*/
    }
}
...
bool read_global_parameters(address_t address) {
    ...
    log_mini_info("%u", 7425);  /* "read global_parameters: starting"*/

    spin1_memcpy(&global_parameters, address, sizeof(global_parameters));

    log_mini_info("%u%08x%08x%u", 7426,global_parameters.key, global_parameters.set_rate_neuron_id_mask,global_parameters.timer_offset);
    /* "\t key = %08x, set rate mask = %08x, timer offset = %u"*/



    log_mini_info("%u%u%u%u%u", 7427, global_parameters.spike_source_seed[0],global_parameters.spike_source_seed[1],global_parameters.spike_source_seed[2],global_parameters.spike_source_seed[3]);
    /* "\t seed = %u %u %u %u"*/
    ...
    log_mini_info("%u%u%u", 7428,global_parameters.n_spike_sources, global_parameters.first_source_id);
    /* "\t spike sources = %u, starting at %u"*/

    log_mini_info("%u%k", 7429,(REAL)(global_parameters.seconds_per_tick));
    /* "seconds_per_tick = %k\n"*/

    log_mini_info("%u%k", 7430, global_parameters.ticks_per_second);  /* "ticks_per_second = %k\n"*/
    log_mini_info("%u%k", 7431,global_parameters.slow_rate_per_tick_cutoff);
    /* "slow_rate_per_tick_cutoff = %k\n"*/


    log_mini_info("%u", 7432);  /* "read_global_parameters: completed successfully"*/
    return true;
}
...
static bool read_poisson_parameters(address_t address) {
    ...
    if (global_parameters.n_spike_sources > 0) {
        ...
        if (poisson_parameters == NULL) {
            log_mini_error("%u", 7433);  /* "Failed to allocate poisson_parameters"*/
            return false;
        }
    ...
    log_mini_info("%u", 7434);  /* "read_poisson_parameters: completed successfully"*/
    return true;
}
...
static bool initialise_recording(){
    ...
    log_mini_info("%u%08x", 7435, recording_flags);  /* "Recording flags = 0x%08x"*/
...
static bool initialize() {
    log_mini_info("%u", 7436);  /* "Initialise: started"*/
    ...
    log_mini_info("%u", 7437);  /* "Initialise: completed successfully"*/
...
void resume_callback() {
    ...
    if (!read_poisson_parameters(
            data_specification_get_region(POISSON_PARAMS, address))){
        log_mini_error("%u", 7438);  /* "failed to reread the Poisson parameters from SDRAM"*/
        rt_error(RTE_SWERR);
    }
    ...
    log_mini_info("%u%u", 7439, time);  /* "Successfully resumed Poisson spike source at time: %u"*/
...
bool store_poisson_parameters() {
    log_mini_info("%u", 7440);  /* "stored_parameters: starting"*/
    ...
    log_mini_info("%u", 7441);  /* "stored_parameters : completed successfully"*/
    return true;
}
...
void _send_spike(uint spike_key, uint timer_count) {
    ...
    // Send the spike
    log_mini_debug("%u%x%d", 7442, spike_key, time);  /* "Sending spike packet %x at %d\n"*/
    ...
    if (recording_flags > 0) {
        if (n_spike_buffers_allocated < n_spikes) {
            ...
            timed_out_spikes *new_spikes = (timed_out_spikes *) spin1_malloc(
                new_size);
            if (new_spikes == NULL) {
                log_mini_error("%u", 7443);  /* "Cannot reallocate spike buffer"*/
                rt_error(RTE_SWERR);
            }
...
void timer_callback(uint timer_count, uint unused) {
    ...
    log_mini_debug("%u%u", 7444, time);  /* "Timer tick %u"*/
    ...
    if (infinite_run != TRUE && time >= simulation_ticks) {
        ...
        if (!store_poisson_parameters()){
            log_mini_error("%u", 7445);  /* "Failed to write poisson parameters to SDRAM"*/
            rt_error(RTE_SWERR);
        }
    ...
    for (index_t s = 0; s < global_parameters.n_spike_sources; s++) {
        ...
        if (spike_source->is_fast_source) {
            if (time >= spike_source->start_ticks
                ...
                log_mini_debug("%u%d", 7446, num_spikes);  /* "Generating %d spikes"*/
...
void set_spike_source_rate(uint32_t id, REAL rate) {
    if ((id >= global_parameters.first_source_id) &&
        ...
        log_mini_debug("%u%u%u%k", 7447, id, sub_id, rate);  /* "Setting rate of %u (%u) to %kHz"*/
...
void c_main(void) {
    ...
    // Load DTCM data
    if (!initialize()) {
        log_mini_error("%u", 7448);  /* "Error in initialisation - exiting!"*/
        rt_error(RTE_SWERR);
    }
