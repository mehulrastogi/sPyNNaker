...
bool read_connection_builder_region(address_t *in_region,
    ...
    *in_region = region;
    ...
    log_mini_debug("%u%u%u", 7041,synaptic_matrix_offset, delayed_synaptic_matrix_offset);
    /* "Synaptic matrix offset = %u, delayed offset = %u"*/
    log_mini_debug("%u%u%u", 7042,max_row_n_synapses, max_delayed_row_n_synapses);
    /* "Max row synapses = %u, max delayed row synapses = %u"*/
    ...
    log_mini_debug("%u%08x%08x", 7043,synaptic_matrix, delayed_synaptic_matrix);
    /* "Generating matrix at 0x%08x, delayed at 0x%08x"*/
    ...
    // If failed, log error
    if (!status) {
        log_mini_error("%u", 7044);  /* "\tMatrix generation failed"*/
        return false;
    }
...
bool read_sdram_data(
    ...
    log_mini_info("%u%u%u%u", 7045,n_in_edges, post_slice_count, post_slice_start);
...
void c_main(void) {
    ...
    log_mini_info("%u", 7046);  /* "Starting To Build Connectors"*/
    ...
    log_mini_info("%u%08x%08x", 7047,params_address, syn_mtx_addr);
    /* "\tReading SDRAM at 0x%08x, writing to matrix at 0x%08x"*/

    // Run the expander
    if (!read_sdram_data(
            (address_t) params_address, (address_t) syn_mtx_addr)) {
        log_mini_info("%u", 7048);  /* "!!!   Error reading SDRAM data   !!!"*/
        rt_error(RTE_ABORT);
    }

    log_mini_info("%u", 7049);  /* "Finished On Machine Connectors!"*/
}
