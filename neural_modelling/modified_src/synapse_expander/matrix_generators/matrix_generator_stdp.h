...
void matrix_generator_stdp_write_row(
    ...
    for (uint32_t synapse = 0; synapse < n_synapses; synapse++) {
        ...
        if (pp_address[delay.stage] == NULL) {
            log_mini_error("%u%u", 7065, delay.stage);  /* "Delay stage %u has not been initialised"*/
            rt_error(RTE_SWERR);
        }
        ...
        if (space_half_words[delay.stage] <
            ...
            log_mini_warning("%u%u%u%u", 7066,delay.stage, space_half_words[delay.stage],params->n_half_words_per_pp_synapse);
