...
void matrix_generator_static_write_row(
    ...
    for (uint32_t i = 0; i < max_stage; i++) {
        if (row_address[i] != NULL) {
            log_mini_debug("%u%08x%u", 7067,&(row_address[i][STATIC_FIXED_FIXED_SIZE]), i);
    ...
    for (uint32_t synapse = 0; synapse < n_synapses; synapse++) {
        ...
        if (write_address[delay.stage] == NULL) {
            log_mini_error("%u%u", 7068, delay.stage);  /* "Delay stage %u has not been initialised"*/
            rt_error(RTE_SWERR);
        }

        // Avoid errors when rows are full
        if (space[delay.stage] == 0) {
            log_mini_warning("%u%u", 7069);  /* "Row for delay stage %u is full - word not added!"*/
            continue;
        }
        ...
        // Write the word
        log_mini_debug("%u%08x", 7070, &(write_address[delay.stage][0]));  /* "Writing word to 0x%08x"*/
