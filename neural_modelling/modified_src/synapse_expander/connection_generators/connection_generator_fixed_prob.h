...
void *connection_generator_fixed_prob_initialise(address_t *region) {
    ...
    *region = params_sdram;
    log_mini_debug("%u%u%k", 7058, params->params.allow_self_connections,(accum) params->params.probability);
    /* "Fixed Probability Connector, allow self connections = %u, probability = %k"*/
...
uint32_t connection_generator_fixed_prob_generate(
    ...
    for (uint32_t i = 0; i < post_slice_count; i++) {
        ...
        if ((value <= params->params.probability) &&
            ...
            log_mini_warning("%u", 7059);  /* "Row overflow"*/
        }
    }
