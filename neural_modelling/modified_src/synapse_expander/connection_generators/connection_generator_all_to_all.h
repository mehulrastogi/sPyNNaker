...
void *connection_generator_all_to_all_initialise(address_t *region) {
    ...
    log_mini_debug("%u%u", 7060,params->allow_self_connections);
    /* "All to all connector, allow self connections = %u"*/
...
uint32_t connection_generator_all_to_all_generate(
    ...
    log_mini_debug("%u%u", 7061, pre_neuron_index);  /* "Generating for %u"*/
    ...
    for (uint32_t i = 0; i < post_slice_count; i++) {
        ...
        if (!params->allow_self_connections &&
            ...
            log_mini_debug("%u%u", 7062, post_slice_start + i);  /* "Not generating for post %u"*/
            continue;
        }
        indices[n_conns++] = i;
    }
