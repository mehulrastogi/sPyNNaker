...
void *connection_generator_fixed_total_initialise(address_t *region) {
    ...
    *region = params_sdram;
    log_mini_debug("%u%u%u%u%u", 7054, params->params.allow_self_connections,params->params.with_replacement, params->params.n_connections,params->params.n_potential_synapses);
    /* "Fixed Total Number Connector, allow self connections = %u, with replacement = %u, n connections = %u, n potential connections = %u"*/
...
uint32_t connection_generator_fixed_total_generate(
    ...
    if (n_conns > max_row_length) {
        if (pre_neuron_index == (pre_slice_start + pre_slice_count - 1)) {
            log_mini_warning("%u%u", 7055, n_conns - max_row_length);
            /* "Could not create %u connections"*/
        }
        n_conns = max_row_length;
    }
    log_mini_debug("%u%u%u", 7056,n_conns, params->params.n_connections);
    /* "Generating %u of %u synapses"*/
