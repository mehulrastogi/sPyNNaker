...
matrix_generator_t matrix_generator_init(uint32_t hash, address_t *in_region) {
    ...
    for (uint32_t i = 0; i < N_MATRIX_GENERATORS; i++) {
        ...
        if (hash == matrix_generators[i].hash) {
            ...
            if (generator == NULL) {
                log_mini_error("%u", 7028);  /* "Could not create generator"*/
                return NULL;
            }
    ...
    log_mini_error("%u%u", 7029, hash);  /* "Matrix generator with hash %u not found"*/
    return NULL;
}
...
bool matrix_generator_generate(
    ...
    for (uint32_t pre_neuron_index = pre_slice_start;
        ...
        log_mini_debug("%u%u", 7030, n_indices);  /* "Generated %u synapses"*/
        ...
        for (uint32_t i = 0; i < n_indices; i++) {
            ...
            if (delay != delays[i]) {
                log_mini_debug("%u%k%u", 7031, delay, delays[i]);  /* "Rounded delay %k to %u"*/
            }
        }
        ...
        for (uint32_t i = 0; i < n_indices; i++) {
            ...
            if (weight != weights[i]) {
                log_mini_debug("%u%k%u", 7032, weight, weights[i]);  /* "Rounded weight %k to %u"*/
            }
        }
    ...
    log_mini_debug("%u%u", 7033, n_connections);  /* "\t\tTotal synapses generated = %u. Done!"*/
