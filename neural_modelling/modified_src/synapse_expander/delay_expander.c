...
bool read_delay_builder_region(address_t *in_region,
    ...
    *in_region = region;
    ...
    for (uint32_t pre_neuron_index = pre_slice_start;
        ...
        log_mini_debug("%u%u", 7034, n_indices);  /* "Generated %u synapses"*/
        ...
        for (uint32_t i = 0; i < n_indices; i++) {
            ...
            if (delay != rounded_delay) {
                log_mini_debug("%u%k%u", 7035, delay, rounded_delay);  /* "Rounded delay %k to %u"*/
            }
...
bool read_sdram_data(
    ...
    log_mini_debug("%u%u%u%u", 7036,n_out_edges, pre_slice_count, pre_slice_start);
    /* "Generating %u delay edges for %u atoms starting at %u"*/
...
void c_main(void) {
    ...
    log_mini_info("%u", 7037);  /* "Starting To Build Delays"*/
    ...
    log_mini_info("%u%08x%08x", 7038,delay_params_address, params_address);
    /* "\tReading SDRAM delay params at 0x%08x, expander params at 0x%08x"*/


    // Run the expander
    if (!read_sdram_data(
            (address_t) delay_params_address, (address_t) params_address)) {
        log_mini_info("%u", 7039);  /* "!!!   Error reading SDRAM data   !!!"*/
        rt_error(RTE_ABORT);
    }

    log_mini_info("%u", 7040);  /* "Finished On Machine Delays!"*/
}
