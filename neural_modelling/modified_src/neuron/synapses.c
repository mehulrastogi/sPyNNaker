...
static inline void _print_synaptic_row(synaptic_row_t synaptic_row) {
#if LOG_LEVEL >= LOG_DEBUG
    log_mini_debug("%u%08x%u", 7117,(uint32_t )synaptic_row, synapse_row_plastic_size(synaptic_row));
    /* "Synaptic row, at address %08x Num plastic words:%u\n"*/
    if (synaptic_row == NULL) {
        return;
    }
    log_mini_debug("%u", 7118);  /* "----------------------------------------\n"*/
    ...
    size_t n_fixed_synapses = synapse_row_num_fixed_synapses(
        fixed_region_address);
    log_mini_debug("%u%u%u", 7119,n_fixed_synapses,synapse_row_num_plastic_controls(fixed_region_address));
    ...
    for (uint32_t i = 0; i < n_fixed_synapses; i++) {
        ...
        log_mini_debug("%u%08x%3d%5u", 7120, synapse, i,synapse_row_sparse_weight(synapse));
        /* "%08x [%3d: (w: %5u (="*/
        synapses_print_weight(synapse_row_sparse_weight(synapse),
                              ring_buffer_to_input_left_shifts[synapse_type]);
        log_mini_debug("%u%2u%s%3u%08x%08x", 7121,synapse_row_sparse_delay(synapse, synapse_type_index_bits),_get_type_char(synapse_type),synapse_row_sparse_index(synapse, synapse_index_mask),SYNAPSE_DELAY_MASK, synapse_type_index_bits);
...
bool synapses_initialise(
    ...
    log_mini_debug("%u", 7126);  /* "synapses_initialise: starting"*/
    ...
    *ring_buffer_to_input_buffer_left_shifts =
    ...
    log_mini_debug("%u%d", 7127, direct_matrix_size);  /* "Direct matrix malloc size is %d"*/
    ...
    if (direct_matrix_size != 0) {
        *direct_synapses_address = (address_t)
        ...
        if (*direct_synapses_address == NULL) {
            log_mini_error("%u", 7128);  /* "Not enough memory to allocate direct matrix"*/
            return false;
        }
        log_mini_debug("%u%u%08x", 7129,direct_matrix_size, *direct_synapses_address);
    ...
    log_mini_debug("%u", 7130);  /* "synapses_initialise: completed successfully"*/
    ...
    if (ring_buffers == NULL) {
        log_mini_error("%u%u", 7131, ring_buffer_size);
