...
static bool initialise_recording(address_t recording_address){
    ...
    log_mini_debug("%u%08x", 7104, recording_flags);  /* "Recording flags = 0x%08x"*/
    return success;
}

void c_main_store_provenance_data(address_t provenance_region){
    log_mini_debug("%u", 7105);  /* "writing other provenance data"*/
    ...
    log_mini_debug("%u", 7106);  /* "finished other provenance data"*/
}
...
static bool initialise() {
    log_mini_debug("%u", 7107);  /* "Initialise: started"*/
    ...
    log_mini_debug("%u", 7108);  /* "Initialise: finished"*/
    return true;
}
...
void resume_callback() {
    ...
    if (!neuron_reload_neuron_parameters(
        ...
        log_mini_error("%u", 7109);  /* "failed to reload the neuron parameters."*/
        rt_error(RTE_SWERR);
    }
}
...
void timer_callback(uint timer_count, uint unused) {
    ...
    log_mini_debug("%u%u", 7110, time);  /* "Timer tick %u \n"*/
    ...
    if (infinite_run != TRUE && time >= simulation_ticks) {
        ...
        log_mini_debug("%u", 7111);  /* "Completed a run"*/
        ...
        if (recording_flags > 0) {
            log_mini_debug("%u", 7112);  /* "updating recording regions"*/
            recording_finalise();
        }
        profiler_finalise();
        ...
        log_mini_debug("%u%d", 7113, count_rewires);  /* "Rewire tries = %d"*/
...
//        // flag pipeline as busy and trigger a feed event
        if (!get_dma_busy()) {
            log_mini_debug("%u", 7114);  /* "Sending user event for new spike"*/
            if (spin1_trigger_user_event(0, 0)) {
                set_dma_busy(true);
            } else {
                log_mini_debug("%u", 7115);  /* "Could not trigger user event\n"*/
            }
        }
        // enable interrupts
...
void c_main(void) {
    ...
    log_mini_debug("%u%d", 7116,timer_period);
