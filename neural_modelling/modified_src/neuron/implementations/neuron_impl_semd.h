// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/implementations/neuron_impl_semd.h
...
static bool neuron_impl_initialise(uint32_t n_neurons) {
    ...
    if (neuron_array == NULL) {
        log_mini_error("%u", 7289);  /* "Unable to allocate neuron array - Out of DTCM"*/
        return false;
    }
    ...
    if (input_type_array == NULL) {
        log_mini_error("%u", 7290);  /* "Unable to allocate input type array - Out of DTCM"*/
        return false;
    }
    ...
    if (threshold_type_array == NULL) {
        log_mini_error("%u", 7291);  /* "Unable to allocate threshold type array - Out of DTCM"*/
        return false;
    }
...
static void neuron_impl_load_neuron_parameters(
    ...
    log_mini_debug("%u%u%u", 7293,next, n_neurons);
    /* "writing parameters, next is %u, n_neurons is %u "*/

    log_mini_debug("%u", 7294);  /* "writing neuron local parameters"*/
    spin1_memcpy(neuron_array, &address[next], n_neurons * sizeof(neuron_t));
    next += (n_neurons * sizeof(neuron_t)) / 4;

    log_mini_debug("%u", 7295);  /* "writing input type parameters"*/
    spin1_memcpy(input_type_array, &address[next],
            n_neurons * sizeof(input_type_current_semd_t));
    next += (n_neurons * sizeof(input_type_current_semd_t)) / 4;

    log_mini_debug("%u", 7296);  /* "writing threshold type parameters"*/
    spin1_memcpy(threshold_type_array, &address[next],
           n_neurons * sizeof(threshold_type_t));
    next += (n_neurons * sizeof(threshold_type_t)) / 4;

    log_mini_debug("%u", 7297);  /* "writing synapse parameters"*/
...
static void neuron_impl_store_neuron_parameters(
        address_t address, uint32_t next, uint32_t n_neurons) {
    log_mini_debug("%u", 7298);  /* "writing parameters"*/

    log_mini_debug("%u", 7299);  /* "writing neuron local parameters"*/
    spin1_memcpy(&address[next], neuron_array,
            n_neurons * sizeof(neuron_t));
    next += (n_neurons * sizeof(neuron_t)) / 4;

    log_mini_debug("%u", 7300);  /* "writing input type parameters"*/
    spin1_memcpy(&address[next], input_type_array,
            n_neurons * sizeof(input_type_current_semd_t));
    next += (n_neurons * sizeof(input_type_current_semd_t)) / 4;

    log_mini_debug("%u", 7301);  /* "writing threshold type parameters"*/
    spin1_memcpy(&address[next], threshold_type_array,
            n_neurons * sizeof(threshold_type_t));
    next += (n_neurons * sizeof(threshold_type_t)) / 4;

    log_mini_debug("%u", 7302);  /* "writing synapse parameters"*/
		...
		for (index_t i = 0; i < n_neurons; i++) {
			input_t input =
				...
				- synapse_types_get_inhibitory_input(
					&(neuron_synapse_shaping_params[i]));
			if (bitsk(input) != 0) {
    log_mini_debug("%u%3u%12.6k", 7304, i, input);  /* "%3u: %12.6k (= "*/
				synapse_types_print_input(
					&(neuron_synapse_shaping_params[i]));
    log_mini_debug("%u", 7305);  /* ")\n"*/
			}
		}
  log_mini_debug("%u", 7306);  /* "-------------------------------------\n"*/
	}
}

void neuron_impl_print_synapse_parameters(uint32_t n_neurons) {
 log_mini_debug("%u", 7307);  /* "-------------------------------------\n"*/
	for (index_t n = 0; n < n_neurons; n++) {
	    synapse_types_print_parameters(&(neuron_synapse_shaping_params[n]));
	}
 log_mini_debug("%u", 7308);  /* "-------------------------------------\n"*/
}
