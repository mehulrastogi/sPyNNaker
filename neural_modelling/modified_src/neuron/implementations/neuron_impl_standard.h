...
static bool neuron_impl_initialise(uint32_t n_neurons) {
    ...
    if (sizeof(global_neuron_params_t) > 0) {
        ...
        if (global_parameters == NULL) {
            log_mini_error("%u", 7263);
            /* "Unable to allocate global neuron parameters- Out of DTCM"*/
            return false;
        }
    }
    ...
    if (sizeof(neuron_t) != 0) {
        ...
        if (neuron_array == NULL) {
            log_mini_error("%u", 7264);  /* "Unable to allocate neuron array - Out of DTCM"*/
            return false;
        }
    }
    ...
    if (sizeof(input_type_t) != 0) {
        ...
        if (input_type_array == NULL) {
            log_mini_error("%u", 7265);  /* "Unable to allocate input type array - Out of DTCM"*/
            return false;
        }
    }
    ...
    if (sizeof(additional_input_t) != 0) {
        ...
        if (additional_input_array == NULL) {
            log_mini_error("%u", 7266);
            /* "Unable to allocate additional input array - Out of DTCM"*/
            return false;
        }
    }
    ...
    if (sizeof(threshold_type_t) != 0) {
        ...
        if (threshold_type_array == NULL) {
            log_mini_error("%u", 7267);  /* "Unable to allocate threshold type array - Out of DTCM"*/
            return false;
        }
    }
    ...
    if (sizeof(synapse_param_t) != 0) {
        ...
        if (neuron_synapse_shaping_params == NULL) {
            log_mini_error("%u", 7268);
            /* "Unable to allocate synapse parameters array - Out of DTCM"*/
            return false;
        }
    }
...
static void neuron_impl_load_neuron_parameters(
    ...
    log_mini_debug("%u%u%u", 7269,next, n_neurons);
    /* "reading parameters, next is %u, n_neurons is %u "*/
    ...
    neuron_model_set_global_neuron_params(global_parameters);
        ...
        log_mini_debug("%u", 7275);  /* "-------------------------------------\n"*/
        for (index_t n = 0; n < n_neurons; n++) {
            neuron_model_print_parameters(&neuron_array[n]);
        }
        log_mini_debug("%u", 7276);  /* "-------------------------------------\n"*/
        //}
...
static void neuron_impl_store_neuron_parameters(
        address_t address, uint32_t next, uint32_t n_neurons) {
    log_mini_debug("%u", 7277);  /* "writing parameters"*/
    ...
    log_mini_debug("%u", 7278);  /* "writing neuron local parameters"*/
    spin1_memcpy(&address[next], neuron_array,
            n_neurons * sizeof(neuron_t));
    next += ((n_neurons * sizeof(neuron_t)) + 3) / 4;

    log_mini_debug("%u", 7279);  /* "writing input type parameters"*/
    spin1_memcpy(&address[next], input_type_array,
            n_neurons * sizeof(input_type_t));
    next += ((n_neurons * sizeof(input_type_t)) + 3) / 4;

    log_mini_debug("%u", 7280);  /* "writing threshold type parameters"*/
    spin1_memcpy(&address[next], threshold_type_array,
            n_neurons * sizeof(threshold_type_t));
    next += ((n_neurons * sizeof(threshold_type_t)) + 3) / 4;

    log_mini_debug("%u", 7281);  /* "writing synapse parameters"*/
    spin1_memcpy(&address[next], neuron_synapse_shaping_params,
            n_neurons * sizeof(synapse_param_t));
    next += ((n_neurons * sizeof(synapse_param_t)) + 3) / 4;

    log_mini_debug("%u", 7282);  /* "writing additional input type parameters"*/
		...
		for (index_t i = 0; i < n_neurons; i++) {
			input_t input =
				...
				- synapse_types_get_inhibitory_input(
					&(neuron_synapse_shaping_params[i]));
			if (bitsk(input) != 0) {
    log_mini_debug("%u%3u%12.6k", 7284, i, input);  /* "%3u: %12.6k (= "*/
				synapse_types_print_input(
					&(neuron_synapse_shaping_params[i]));
    log_mini_debug("%u", 7285);  /* ")\n"*/
			}
		}
  log_mini_debug("%u", 7286);  /* "-------------------------------------\n"*/
	}
}

void neuron_impl_print_synapse_parameters(uint32_t n_neurons) {
 log_mini_debug("%u", 7287);  /* "-------------------------------------\n"*/
	for (index_t n = 0; n < n_neurons; n++) {
	    synapse_types_print_parameters(&(neuron_synapse_shaping_params[n]));
	}
 log_mini_debug("%u", 7288);  /* "-------------------------------------\n"*/
}
