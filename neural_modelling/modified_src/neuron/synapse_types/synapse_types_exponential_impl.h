...
static inline const char *synapse_types_get_type_char(
    ...
    if (synapse_type_index == EXCITATORY) {
        ...
        log_mini_debug("%u%i", 7358, synapse_type_index);  /* "did not recognise synapse type %i"*/
        return "?";
    }
}
...
static inline void synapse_types_print_parameters(
        synapse_param_pointer_t parameter) {
    log_mini_debug("%u%R", 7359, (unsigned fract) parameter->exc.decay);  /* "exc_decay = %R\n"*/
    log_mini_debug("%u%R", 7360, (unsigned fract) parameter->exc.init);  /* "exc_init  = %R\n"*/
    log_mini_debug("%u%R", 7361, (unsigned fract) parameter->inh.decay);  /* "inh_decay = %R\n"*/
    log_mini_debug("%u%R", 7362, (unsigned fract) parameter->inh.init);  /* "inh_init  = %R\n"*/
    log_mini_debug("%u%11.4k", 7363,parameter->exc.synaptic_input_value);
    /* "gsyn_excitatory_initial_value = %11.4k\n"*/
    log_mini_debug("%u%11.4k", 7364,parameter->inh.synaptic_input_value);
