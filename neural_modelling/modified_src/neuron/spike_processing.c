...
static inline void _setup_synaptic_dma_write(uint32_t dma_buffer_index) {
    ...
    log_mini_debug("%u%u%08x", 7094,n_plastic_region_bytes, buffer->sdram_writeback_address + 1);
    /* "Writing back %u bytes of plastic region to %08x"*/
...
void _multicast_packet_received_callback(uint key, uint payload) {
    use(payload);
    any_spike = true;
    log_mini_debug("%u%x%d%d", 7095, key, time, dma_busy);  /* "Received spike %x at %d, DMA Busy = %d"*/
    ...
    if (in_spikes_add_spike(key)) {
        ...
        if (!dma_busy) {
            ...
            log_mini_debug("%u", 7096);  /* "Sending user event for new spike"*/
            if (spin1_trigger_user_event(0, 0)) {
                dma_busy = true;
            } else {
                log_mini_debug("%u", 7097);  /* "Could not trigger user event\n"*/
            }
        }
    } else {
        log_mini_debug("%u", 7098);  /* "Could not add spike"*/
    }
}
...
void _dma_complete_callback(uint unused, uint tag) {
    ...
    log_mini_debug("%u%u%u", 7099, time, tag);  /* "DMA transfer complete at time %u with tag %u"*/
    ...
    do {
        ...
        if (!synapses_process_synaptic_row(time, current_buffer->row,
            ...
            log_mini_error("%u%.8x%.8x%.8x", 7100,current_buffer->originating_spike,current_buffer->sdram_writeback_address,current_buffer->row);
            ...
            for (uint32_t i = 0;
                ...
                log_mini_error("%u%u%.8x", 7101, i, current_buffer->row[i]);  /* "%u: 0x%.8x"*/
            }
...
bool spike_processing_initialise(
    ...
    for (uint32_t i = 0; i < N_DMA_BUFFERS; i++) {
        ...
        if (dma_buffers[i].row == NULL) {
            log_mini_error("%u", 7102);  /* "Could not initialise DMA buffers"*/
            return false;
        }
        log_mini_debug("%u%u%08x", 7103, i, dma_buffers[i].row);
