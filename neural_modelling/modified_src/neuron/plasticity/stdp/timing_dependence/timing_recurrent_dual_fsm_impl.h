...
static inline post_trace_t timing_add_post_spike(
    ...
    log_mini_debug("%u%d%u", 7160, random,window_length);
...
static inline pre_trace_t timing_add_pre_spike(
    ...
    log_mini_debug("%u%d%u", 7161, random,window_length);
...
static inline update_state_t timing_apply_pre_spike(
    ...
    log_mini_debug("%u%u%u", 7162,time_since_last_post, last_post_trace);
    ...
    if (time_since_last_post > 0) {
        ...
        if (time_since_last_post < last_post_trace) {
            ...
            if (previous_state.accumulator >
                ...
                log_mini_debug("%u%d", 7163,previous_state.accumulator);
                /* "\t\t\t\tDecrementing accumulator=%d"*/
            } else {

                // Otherwise, reset accumulator and apply depression
                log_mini_debug("%u", 7164);  /* "\t\t\t\tApplying depression"*/
...
static inline update_state_t timing_apply_post_spike(
    ...
    log_mini_debug("%u%u%u", 7165,time_since_last_pre, last_pre_trace);
    ...
    if (time_since_last_pre > 0) {
        ...
        if (time_since_last_pre < last_pre_trace) {
            ...
            if (previous_state.accumulator
                ...
                // increment it
                previous_state.accumulator++;
                log_mini_debug("%u%d", 7166,previous_state.accumulator);
                /* "\t\t\t\tIncrementing accumulator=%d"*/
            } else {

                // Otherwise, reset accumulator and apply potentiation
                log_mini_debug("%u", 7167);  /* "\t\t\t\tApplying potentiation"*/
