...
static inline post_trace_t timing_add_post_spike(
    ...
    log_mini_debug("%u%u", 7221, time - last_time);  /* "\tdelta_time=%u\n"*/
...
static inline pre_trace_t timing_add_pre_spike(
    ...
    log_mini_debug("%u%u", 7222, time - last_time);  /* "\tdelta_time=%u\n"*/
...
static inline update_state_t timing_apply_pre_spike(
    ...
    if (time_since_last_post > 0) {
        ...
        log_mini_debug("%u%u%d", 7223,time_since_last_post, decayed_o1);
...
static inline update_state_t timing_apply_post_spike(
    ...
    if (time_since_last_pre > 0) {
        ...
        log_mini_debug("%u%u%d", 7224,time_since_last_pre, decayed_r1);
        /* "\t\t\ttime_since_last_pret=%u, decayed_r1=%d\n"*/

        log_mini_debug("%u%u%d", 7225,time_since_last_post, decayed_r1);
        /* "\t\t\ttime_since_last_post=%u, decayed_r1=%d\n"*/

        if (time_since_last_post < time_since_last_pre){
            log_mini_debug("%u", 7226);  /* "\t\t\tSetting trace to zero as not first pre-post pairing"*/
            decayed_r1 = 0;
        }
