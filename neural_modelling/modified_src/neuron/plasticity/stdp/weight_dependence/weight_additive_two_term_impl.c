...
address_t weight_initialise(address_t address, uint32_t n_synapse_types,
    ...
    log_mini_debug("%u", 7242);  /* "weight_initialise: starting"*/
    log_mini_debug("%u", 7243);  /* "\tSTDP additive two-term weight dependance"*/
    ...
    if (plasticity_weight_region_data == NULL) {
        log_mini_error("%u", 7244);  /* "Could not initialise weight region data"*/
        return NULL;
    }
    for (uint32_t s = 0; s < n_synapse_types; s++) {
        ...
        log_mini_debug("%u%u%d%d%d%d%d%d", 7245,s, plasticity_weight_region_data[s].min_weight,plasticity_weight_region_data[s].max_weight,plasticity_weight_region_data[s].a2_plus,plasticity_weight_region_data[s].a2_minus,plasticity_weight_region_data[s].a3_plus,plasticity_weight_region_data[s].a3_minus);
        /* "\tSynapse type %u: Min weight:%d, Max weight:%d, A2+:%d, A2-:%d, A3+:%d, A3-:%d"*/
    ...
    log_mini_debug("%u", 7246);  /* "weight_initialise: completed successfully"*/
