...
static inline final_state_t _plasticity_update_synapse(
    ...
    log_mini_debug("%u%u", 7140, time);  /* "\tPerforming deferred synapse update at time:%u"*/
    log_mini_debug("%u%u%u%u%u", 7141,window_begin_time, window_end_time, post_window.prev_time,post_window.num_events);
    /* "\t\tbegin_time:%u, end_time:%u - prev_time:%u, num_events:%u"*/
    ...
    while (post_window.num_events > 0) {
        const uint32_t delayed_post_time = *post_window.next_time
                                           + delay_dendritic;
        log_mini_debug("%u%u", 7142,delayed_post_time);
    ...
    log_mini_debug("%u%u%u", 7143,delayed_pre_time, post_window.prev_time);
...
void synapse_dynamics_print_plastic_synapses(
    ...
    log_mini_debug("%u%u", 7144, plastic_synapse);  /* "Plastic region %u synapses\n"*/
    ...
    for (uint32_t i = 0; i < plastic_synapse; i++) {
        ...
        log_mini_debug("%u%08x%3d%5u", 7145, control_word, i, weight);  /* "%08x [%3d: (w: %5u (="*/
        synapses_print_weight(
            weight, ring_buffer_to_input_buffer_left_shifts[synapse_type]);
        log_mini_debug("%u%2u%s%3u%08x%08x", 7146,synapse_row_sparse_delay(control_word, synapse_type_index_bits),synapse_types_get_type_char(synapse_type),synapse_row_sparse_index(control_word, synapse_index_mask),SYNAPSE_DELAY_MASK, synapse_type_index_bits);
...
bool synapse_dynamics_process_plastic_synapses(
    ...
    log_mini_debug("%u%u", 7147, time);  /* "Adding pre-synaptic event to trace at time:%u"*/
...
void synapse_dynamics_process_post_synaptic_event(
    ...
    log_mini_debug("%u%u", 7148, time);  /* "Adding post-synaptic event to trace at time:%u"*/
