...
static inline post_event_history_t *post_events_init_buffers(
    ...
    if (post_event_history == NULL) {
        log_mini_error("%u", 7135);
        /* "Unable to allocate global STDP structures - Out of DTCM: Try reducing the number of neurons per core to fix this problem "*/
...
static inline void print_event_history(post_event_history_t *events){
 log_mini_debug("%u", 7136);  /* "		##  printing entire post event history  ##"*/
	for (uint i = 0; i <= events->count_minus_one; i++){
  log_mini_debug("%u%u%u%u", 7137,i,events->times[i],events->traces[i]);
...
static inline void print_delayed_window_events(post_event_history_t *post_event_history,
 ...
 log_mini_debug("%u", 7138);  /* "		##  printing post window  ##"*/
    ...
    while (post_window.num_events > 0) {
    	const uint32_t delayed_post_time = *post_window.next_time
    	                                           + delay_dendritic;
     log_mini_debug("%u%u%u%u", 7139,post_window.num_events, delayed_post_time,*post_window.next_trace);
     /* "post spike: %u, time: %u, trace: %u"*/
