// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/population_table/population_table_fixed_impl.c
...
static inline void _print_master_population_table() {
    ...
    log_mini_debug("%u", 7328);  /* "master_population\n"*/
    log_mini_debug("%u", 7329);  /* "------------------------------------------\n"*/
    for (uint32_t i = 0; i < MASTER_POPULATION_MAX; i++) {
        ...
        if (row_table_entry != 0) {
            log_mini_debug("%u%d%4u%04x%3u", 7330,i, entry, entry >> 3, row_size_table[row_table_entry]);
            /* "index %d, entry: %4u (13 bits = %04x), size = %3u\n"*/
        }
    }
    log_mini_debug("%u", 7331);  /* "------------------------------------------\n"*/
#endif // LOG_LEVEL >= LOG_DEBUG
}

static inline void _print_row_size_table() {
#if LOG_LEVEL >= LOG_DEBUG
    log_mini_debug("%u", 7332);  /* "row_size_table\n"*/
    log_mini_debug("%u", 7333);  /* "------------------------------------------\n"*/
    for (uint32_t i = 0; i < ROW_SIZE_TABLE_MAX; i++) {
        log_mini_debug("%u%2u%3u", 7334, i, row_size_table[i]);  /* "  index %2u, size = %3u\n"*/
    }
    log_mini_debug("%u", 7335);  /* "------------------------------------------\n"*/
#endif // LOG_LEVEL >= LOG_DEBUG
}
...
bool population_table_initialise(address_t table_address,
    ...
    log_mini_debug("%u", 7336);  /* "population_table_initialise: starting"*/
    // Copy the master population table
    log_mini_debug("%u%.8x", 7337, table_address);  /* "reading master pop table from address 0x%.8x"*/
    spin1_memcpy(master_population_table, table_address,
            MASTER_POPULATION_MAX * sizeof(uint16_t));

    // Store the base address
    log_mini_debug("%u%.8x", 7338,synapse_rows_address);
    ...
    log_mini_debug("%u%d%.8x", 7339,ROW_SIZE_TABLE_MAX * sizeof(uint32_t),table_address + ((MASTER_POPULATION_MAX * sizeof(uint16_t)) / 4));
    /* "reading row length table of %d bytes from mem address 0x%.8x"*/
    ...
    *row_max_n_words = row_size_table[ROW_SIZE_TABLE_MAX - 1]
    ...
    log_mini_debug("%u", 7340);  /* "population_table_initialise: completed successfully"*/
...
bool population_table_get_first_address(
    ...
    log_mini_debug("%u%08x%u%u%u%u", 7341,spike, table_index, row_size_index, address_offset, neuron_id);
    /* "spike = %08x, table_index = %u, row_size_index = %u, address_offset = %u, neuron_id = %u"*/


    // If the row size is 0, there is no entry
    if (row_size_index == 0) {
        log_mini_debug("%u%u%x", 7342,spike, spike);
    ...
    *n_bytes_to_transfer = (num_synaptic_words + N_SYNAPSE_ROW_HEADER_WORDS)
    ...
    log_mini_debug("%u%u%u%u%08x%u", 7343, stride, neuron_offset,population_offset, synaptic_rows_base_address,*n_bytes_to_transfer);
    /* "stride = %u, neuron offset = %u, population offset = %u, base = %08x, size = %u"*/
