...
{
    log_mini_debug("%u", 7369);  /* "SR init."*/
    log_mini_debug("%u", 7370);  /* "Registering DMA callback"*/
    simulation_dma_transfer_done_callback_on(
        DMA_TAG_READ_SYNAPTIC_ROW_FOR_REWIRING,
        synaptic_row_restructure);
    log_mini_debug("%u", 7371);  /* "Callback registered"*/
    ...
    if (rewiring_dma_buffer.row == NULL) {
        log_mini_error("%u", 7372);  /* "Fail init DMA buffers"*/
        rt_error(RTE_SWERR);
    }
    ...
    log_mini_debug("%u", 7373);  /* "SR init complete."*/
...
void synaptogenesis_dynamics_rewire(uint32_t time)
{
    ...
    if (!element_exists && !rewiring_data.random_partner) {
        ...
        if (_spike == ANY_SPIKE) {
            log_mini_debug("%u", 7374);  /* "No previous spikes"*/
            _setup_synaptic_dma_read();
            return;
        }
    ...
    if (!population_table_get_first_address(_spike, &synaptic_row_address,
        &n_bytes)) {
        log_mini_error("%u%d", 7375, _spike);  /* "FAIL@key %d"*/
        rt_error(RTE_SWERR);
    }
    ...
    while (!spin1_dma_transfer(
        ...
        log_mini_error("%u", 7376);  /* "DMA queue full-read"*/
    }
...
bool synaptogenesis_dynamics_formation_rule(void)
{
    ...
    if (no_elems >= rewiring_data.s_max) {
        log_mini_debug("%u", 7378);  /* "row is full"*/
        return false;
    }
