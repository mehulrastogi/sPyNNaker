...
static bool _neuron_load_neuron_parameters(address_t address) {
    ...
    log_mini_debug("%u", 7077);  /* "loading parameters"*/
...
bool neuron_reload_neuron_parameters(address_t address){
    log_mini_debug("%u", 7078);  /* "neuron_reloading_neuron_parameters: starting"*/
...
bool neuron_initialise(address_t address, uint32_t *n_neurons_value,
    ...
    log_mini_debug("%u", 7079);  /* "neuron_initialise: starting"*/

    *timer_offset = address[TIMER_START_OFFSET];
    time_between_spikes = address[TIME_BETWEEN_SPIKES] * sv->cpu_clk;
    log_mini_debug("%u%u%u", 7080,*timer_offset, time_between_spikes);
    ...
    if (!use_key){
        log_mini_debug("%u", 7081);  /* "\tThis model is not expecting to transmit as it has no key"*/
    } else{
        log_mini_debug("%u%08x", 7082, key);  /* "\tThis model is expected to transmit with key = %08x"*/
    }
    ...
    *incoming_spike_buffer_size = address[INCOMING_SPIKE_BUFFER_SIZE];
    ...
    log_mini_debug("%u%u%u", 7083, n_neurons,*incoming_spike_buffer_size);
    ...
    for (uint32_t i = 0; i < n_recorded_vars; i++) {
        ...
        if (var_recording_values[i] == NULL) {
            log_mini_error("%u%d", 7091, i);
            /* "Could not allocate space for var_recording_values[%d]"*/
            return false;
        }
    }
...
void neuron_do_timestep_update(
    ...
    for (index_t neuron_index = 0; neuron_index < n_neurons; neuron_index++) {
        ...
        if (spike) {
            log_mini_debug("%u%u%u", 7092, neuron_index, time);  /* "neuron %u spiked at time %u"*/
            ...
            log_mini_debug("%u%d", 7093,neuron_index);
